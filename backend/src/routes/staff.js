...
router.post('/', async (req, res) => {
  try {
    ...
    emitRealtimeEvent('staff', 'created', { id: rows[0].id });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    emitRealtimeEvent('staff', 'updated', { id: rows[0]?.id || Number(req.params.id) });
    res.json(rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    emitRealtimeEvent('staff', 'deleted', { id: Number(req.params.id) });
